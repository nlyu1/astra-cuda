...
def analyze_triangle_distribution(center, half_width, epsilon_fullsupport, epsilon_uniform, 
                                min_value, max_value, title=""):
    """
    Plot the triangle-based action distribution (hierarchical mixture model).
    
    Args:
        center: float in (0, 1), mode of distributions
        half_width: float in (0, 0.5), width of concentrated distribution  
        epsilon_fullsupport: float in (0, 1), prob of using full-support distribution
        epsilon_uniform: float in (0, 1), uniform mixing weight in full-support dist
        min_value: int, minimum discrete value
        max_value: int, maximum discrete value
        title: str, title for the plot
    """
    import sys
    ...
    epsilon_fs_t = torch.tensor([epsilon_fullsupport], dtype=torch.float32)
    ...
    # Transform to actual value range for visualization  
    x_continuous_scaled = x_continuous.numpy() * (max_value - min_value + 1) + (min_value - 0.5)
    
    # Calculate discrete probabilities
    discrete_values = np.arange(min_value, max_value + 1)
    discrete_probs = []
    rangeP1 = max_value - min_value + 1
    
    for val in discrete_values:
        # Calculate interval bounds
        unit_lb = ((val - 0.5) + 0.5 - min_value) / rangeP1
        unit_ub = ((val + 0.5) + 0.5 - min_value) / rangeP1
    ...
    fig.add_trace(go.Scatter(
        x=x_continuous_scaled,
        y=y_continuous / rangeP1,  # Adjust density for scaled x-axis
        mode='lines',
        name='Triangle PDF (scaled)',
        line=dict(color='blue', width=2)
    ))
    ...
    # Update layout
    fig.update_layout(
        title=f'{title} (c={center:.3f}, hw={half_width:.3f}, εfs={epsilon_fullsupport:.3f}, εu={epsilon_uniform:.3f}, H={entropy_value:.2f})',
        xaxis_title='Value',
        yaxis_title='Probability Density',
        xaxis=dict(
            range=[min_value - 1, max_value + 1],
            dtick=1 if (max_value - min_value) <= 10 else None
        ),
        showlegend=True,
        hovermode='x unified'
    )
    ...
    return {
        ...
        'epsilon_fullsupport': epsilon_fullsupport,
