...
__all__ = [
    ...
    'Gaussian', 'Player', 'Game', 'History', 'aggregate_duplicate_updates'
]
...
def aggregate_duplicate_updates(posteriors):
      """
      Aggregate multiple posterior updates for the same agent.
      Uses precision-weighted averaging.
      """
      # Precision-weighted average
      precisions = [p.pi for p in posteriors]  # pi = 1/sigma^2
      total_precision = sum(precisions)

      # Weighted mean
      weighted_means = [p.mu * p.pi for p in posteriors]
      mu = sum(weighted_means) / total_precision

      # Combined uncertainty
      sigma = 1.0 / np.sqrt(total_precision)
