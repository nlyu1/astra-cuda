...
class RolloutGenerator:
    def __init__(self, args):
        ...
        self.agents = [
            ...
            for _ in range(args.get_game_config()['players'])]
        self.payoff_matrix = torch.zeros(args.get_game_config()['players'], 4 + 1, 2)
    ...
    def generate_rollout(self, state_dicts):
        num_players = self.args.get_game_config()['players']
        for j in range(num_players):
            self.agents[j].load_state_dict(state_dicts[j], strict=True)
            self.agents[j].eval()
