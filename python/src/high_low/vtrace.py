...
class HighLowVTraceTrainer:
    def __init__(self, args, agent):
        ...
        self.checkpoint_interval = args.iterations_per_checkpoint
        ...
        self.save_dir = Path("checkpoints") / args.exp_name
        assert self.save_dir.exists(), f"Please mkdir {self.save_dir} before initializing trainer"
    ...
    def save_checkpoint(self, step, name='main'):
        if step < self.last_checkpoint + self.checkpoint_interval:
            return False 
        self.last_checkpoint = step

        checkpoint_path = self.save_dir / f"{name}_{step}.pt"
