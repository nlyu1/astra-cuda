...
def get_uid(obj: object, digits=4) -> str:
    """
    Computes a consistent SHA256 hash for a given Python object.
    Handles PyTorch models/state_dicts, Tensors, and other primitives.
    """
    hasher = hashlib.sha256()

    def update_hash(data):
        if isinstance(data, (collections.OrderedDict, dict)):
            # Handle dictionaries by hashing their items.
            # Sort keys for standard dicts to ensure consistency.
            for key in sorted(data.keys()):
                update_hash(key)
                update_hash(data[key])
        elif isinstance(data, ImpalaBuffer):
            # Handle the buffer by hashing its contents.
            update_dict = data.get_update_dictionary()
            for key in sorted(update_dict.keys()):
                update_hash(update_dict[key])
        elif isinstance(data, torch.Tensor):
            # BASE CASE for Tensors: Hash its bytes.
            tensor_bytes = data.cpu().contiguous().numpy().tobytes()
            hasher.update(tensor_bytes)
        elif isinstance(data, str):
            # BASE CASE for strings: Hash its UTF-8 representation.
            hasher.update(data.encode('utf-8'))
        else:
            # Correctly report the type of the failing data.
            raise TypeError(f"Unsupported type for hashing: {type(data)}")

    update_hash(obj)
    return hasher.hexdigest()[:digits]
...
class HighLowImpalaTrainer:
    ...
    def train(self, update_dictionary):
        ...
        explained_vars, value_losses, pg_losses, entropy_losses, approx_kls, pred_settlement_losses, pred_private_roles_losses = (
