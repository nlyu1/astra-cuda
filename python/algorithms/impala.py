...
class ImpalaBuffer:
    def __init__(self, args, num_features, device='cuda'):
        ...
        self.obs = torch.zeros((args.num_steps, args.num_envs, num_features)).to(device).share_memory_()
        self.actions = torch.zeros((args.num_steps, args.num_envs, 4)).to(device).share_memory_()
        self.logprobs = torch.zeros((args.num_steps, args.num_envs)).to(device).share_memory_()
        self.rewards = torch.zeros((args.num_steps, args.num_envs)).to(device).share_memory_()
        self.dones = torch.zeros((args.num_steps, args.num_envs)).to(device).share_memory_()
...
class HighLowImpalaTrainer:
    ...
    def train(self, update_dictionary):
        ...
        # print('Raw obs shape:', obs.shape, 'raw actions shape:', actions.shape)
        ...
        for _ in range(self.args.update_epochs):
            ...
            for start in range(0, self.args.num_envs, num_envs_per_minibatch):
                ...
                step_results = self._compiled_train_step(
                    obs, logprobs, actions, rewards, dones, minibatch_env_indices)
    ...
    def _train_step(self, obs, logprobs, actions, rewards, dones, minibatch_env_indices):
        ...
        _, new_logprob, entropy, values = self.agent._forward_impl(
