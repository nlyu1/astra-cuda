...
std::string GameParameter::ToReprString() const {
  switch (type_) {
    case Type::kInt:
      return StrCat("GameParameter(int_value=", int_value(), ")");
    case Type::kDouble:
      return StrCat("GameParameter(double_value=", double_value(), ")");
    case Type::kString:
      return StrCat("GameParameter(string_value='", string_value(), "')");
    case Type::kBool:
      return StrCat(
          "GameParameter(bool_value=", bool_value() ? "True" : "False", ")");
    case Type::kUnset:
      return StrCat("GameParameter()");
    case Type::kGame:
      return StrCat("GameParameter(game_value=",
                          GameParametersToString(game_value()));
    default:
      AstraFatalError("Unknown type.");
...
std::string GameParameter::Serialize(const std::string& delimiter) const {
  ...
  return StrCat(GameParameterTypeToString(type_), delimiter, val,
...
GameParameter DeserializeGameParameter(const std::string& data,
  ...
  if (parts[0] == "kUnset") {
    ...
    AstraFatalError(StrCat("Unrecognized type: ", parts[0]));
  }
}
...
std::string SerializeGameParameters(const GameParameters& game_params,
  ...
  for (const auto& key_val : game_params) {
    ...
    serialized_params.push_back(
        StrCat(name, name_delimiter, parameter.Serialize()));
  }

  return StrJoin(serialized_params, parameter_delimeter);
}
...
GameParameters DeserializeGameParameters(
  ...
  std::vector<std::string> parts = StrSplit(data, parameter_delimeter);

  for (const auto& part : parts) {
    if (!part.empty()) {
      std::pair<std::string, std::string> pair =
          StrSplitFirst(part, name_delimiter);
...
std::string GameParametersToString(const GameParameters& game_params) {
  std::string str;
  if (game_params.empty()) {
      return "";
  }
  if (game_params.count("name")) {
      str = game_params.at("name").string_value();
  }
  str.push_back('(');
  bool first = true;
  for (const auto& key_val : game_params) {
...
GameParameter GameParameterFromString(const std::string& str) {
  if (str == "True" || str == "true") {
    ...
    int value;
    bool success = SimpleAtoi(str, &value);
    ASTRA_CHECK_TRUE(success);
    return GameParameter(value);
  } else if (str.find_first_not_of("+-0123456789.") == std::string::npos) {
    double value;
    bool success = SimpleAtod(str, &value);
    ASTRA_CHECK_TRUE(success);
    return GameParameter(value);
  } else if (str.back() == ')') {
...
GameParameters GameParametersFromString(const std::string& game_string) {
  ...
  if (parens > 0) {
      AstraFatalError("Missing closing bracket ')'.");
  }
  return params;
}
...
std::string GameParameterTypeToString(const GameParameter::Type& type) {
  switch (type) {
    ...
    case GameParameter::Type::kGame:
      return "kGame";
    default:
      AstraFatalError("Invalid GameParameter");
  }
}
