...
class GameParameter {
 public:
  ...
  int int_value() const {
    ASTRA_CHECK_TRUE(type_ == Type::kInt);
    return int_value_;
  }

  double double_value() const {
    ASTRA_CHECK_TRUE(type_ == Type::kDouble);
    return double_value_;
  }

  const std::string& string_value() const {
    ASTRA_CHECK_TRUE(type_ == Type::kString);
    return string_value_;
  }

  bool bool_value() const {
    ASTRA_CHECK_TRUE(type_ == Type::kBool);
    return bool_value_;
  }

  const GameParameters& game_value() const {
    ASTRA_CHECK_TRUE(type_ == Type::kGame);
    return game_value_;
  }
...
T ParameterValue(const GameParameters& params, const std::string& key,
  ...
  if (iter == params.end()) {
    if (!default_value.has_value()) {
      AstraFatalError(StrCat("Cannot find parameter and no default "
