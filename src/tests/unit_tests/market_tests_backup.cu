...
inline int32_t tensor_item_options_i32(const torch::Tensor& t) {
    return t.cpu().data_ptr<int32_t>()[0];
}
...
void PrintFillDetails(const FillBatch& fills, int32_t market_id, const std::string& label) {
    auto fill_counts = fills.fill_counts.cpu();
    auto fill_counts_acc = fill_counts.accessor<int32_t, 1>();
    int32_t count = fill_counts_acc[market_id];
    ...
    if (count > 0) {
        ...
        auto prices_acc = fill_prices.accessor<int32_t, 2>();
        auto sizes_acc = fill_sizes.accessor<int32_t, 2>();
        auto customer_acc = fill_customer_ids.accessor<int32_t, 2>();
        auto quoter_acc = fill_quoter_ids.accessor<int32_t, 2>();
...
void TestPartialFills() {
    ...
    const int32_t num_markets = 1;
    VecMarket market(num_markets, 128, 1024, 512);
    auto fills = market.NewFillBatch();
    
    // Set device for tensor creation
    auto device = torch::Device(torch::kCUDA, 0);
    
    // Add a large buy order
    cout << "Adding BUY order: price=50, size=100" << endl;
    {
        torch::Tensor bid_prices = torch::full({1}, 50, torch::TensorOptions().dtype(torch::kInt32).device(device));
        torch::Tensor bid_sizes = torch::full({1}, 100, torch::TensorOptions().dtype(torch::kInt32).device(device));
        torch::Tensor ask_prices = torch::zeros({1}, torch::TensorOptions().dtype(torch::kInt32).device(device));
        torch::Tensor ask_sizes = torch::zeros({1}, torch::TensorOptions().dtype(torch::kInt32).device(device));
        torch::Tensor customer_ids = torch::zeros({1}, torch::TensorOptions().dtype(torch::kInt32).device(device));
    ...
    {
        torch::Tensor bid_prices = torch::zeros({1}, torch::kInt32);
        torch::Tensor bid_sizes = torch::zeros({1}, torch::kInt32);
        torch::Tensor ask_prices = torch::full({1}, 50, torch::kInt32);
        torch::Tensor ask_sizes = torch::full({1}, 30, torch::kInt32);
        torch::Tensor customer_ids = torch::ones({1}, torch::kInt32);
        
        market.AddTwoSidedQuotes(bid_prices, bid_sizes, ask_prices, ask_sizes, customer_ids, fills);
        
        // Check fills
        auto counts = fills.fill_counts.cpu();
        assert(tensor_item_options_i32(counts[0]) == 1);
        
        auto sizes = fills.fill_sizes.cpu();
        assert(tensor_item_options_i32(sizes[0][0]) == 30);
    ...
    cout << "\nAdding another SELL order: price=50, size=80" << endl;
    {
        ...
        torch::Tensor bid_sizes = torch::zeros({1}, torch::kInt32);
        torch::Tensor ask_prices = torch::full({1}, 50, torch::kInt32);
        torch::Tensor ask_sizes = torch::full({1}, 80, torch::kInt32);
        torch::Tensor customer_ids = torch::full({1}, 2, torch::kInt32);
        ...
        auto sizes = fills.fill_sizes.cpu();
        assert(tensor_item_options_i32(sizes[0][0]) == 70);
...
void TestMarketIndependence() {
    ...
    const int32_t num_markets = 5;
    VecMarket market(num_markets, 128, 1024, 512);
    auto fills = market.NewFillBatch();
    
    // Set up different scenarios for each market
    torch::Tensor bid_prices = torch::tensor({10, 20, 30, 40, 50}, torch::kInt32);
    torch::Tensor bid_sizes = torch::tensor({100, 200, 300, 400, 500}, torch::kInt32);
    torch::Tensor ask_prices = torch::tensor({15, 25, 35, 45, 55}, torch::kInt32);
    torch::Tensor ask_sizes = torch::tensor({50, 100, 150, 200, 250}, torch::kInt32);
    torch::Tensor customer_ids = torch::zeros({num_markets}, torch::kInt32);
    ...
    auto counts = fills.fill_counts.cpu();
    for (int32_t i = 0; i < num_markets; ++i) {
        assert(tensor_item_options_i32(counts[i]) == 0);
    }
    cout << "✓ No fills when orders don't cross" << endl;
    
    // Now add crossing orders to specific markets only
    torch::Tensor bid_prices2 = torch::tensor({0, 30, 0, 50, 0}, torch::kInt32);
    torch::Tensor bid_sizes2 = torch::tensor({0, 50, 0, 100, 0}, torch::kInt32);
    torch::Tensor ask_prices2 = torch::tensor({0, 20, 0, 40, 0}, torch::kInt32);
    torch::Tensor ask_sizes2 = torch::tensor({0, 75, 0, 150, 0}, torch::kInt32);
    torch::Tensor customer_ids2 = torch::ones({num_markets}, torch::kInt32);
    ...
    assert(tensor_item_options_i32(counts[0]) == 0);  // Market 0: no crossing
    assert(tensor_item_options_i32(counts[1]) == 1);  // Market 1: should have fill
    assert(tensor_item_options_i32(counts[2]) == 0);  // Market 2: no crossing
    assert(tensor_item_options_i32(counts[3]) == 1);  // Market 3: should have fill
    assert(tensor_item_options_i32(counts[4]) == 0);  // Market 4: no crossing
...
void TestMultipleOrdersSamePrice() {
    ...
    for (int32_t i = 0; i < 3; ++i) {
        torch::Tensor bid_prices = torch::full({1}, 25, torch::kInt32);
        torch::Tensor bid_sizes = torch::full({1}, (i + 1) * 10, torch::kInt32);  // 10, 20, 30
        torch::Tensor ask_prices = torch::zeros({1}, torch::kInt32);
        torch::Tensor ask_sizes = torch::zeros({1}, torch::kInt32);
        torch::Tensor customer_ids = torch::full({1}, i, torch::kInt32);
    ...
    cout << "\nAdding SELL order: price=25, size=60 (should match all buys)" << endl;
    {
        torch::Tensor bid_prices = torch::zeros({1}, torch::kInt32);
        torch::Tensor bid_sizes = torch::zeros({1}, torch::kInt32);
        torch::Tensor ask_prices = torch::full({1}, 25, torch::kInt32);
        torch::Tensor ask_sizes = torch::full({1}, 60, torch::kInt32);
        torch::Tensor customer_ids = torch::full({1}, 3, torch::kInt32);
        
        market.AddTwoSidedQuotes(bid_prices, bid_sizes, ask_prices, ask_sizes, customer_ids, fills);
        
        // Check that all orders were filled
        auto counts = fills.fill_counts.cpu();
        cout << "\nFills generated: " << tensor_item_options_i32(counts[0]) << endl;
        
        // Verify total volume matched
        auto sizes = fills.fill_sizes.cpu();
        auto sizes_acc = sizes.accessor<int32_t, 2>();
        
        int32_t total = 0;
        for (int32_t i = 0; i < tensor_item_options_i32(counts[0]); ++i) {
            total += sizes_acc[0][i];
        }
...
void TestBBO() {
    ...
    torch::Tensor bid_prices = torch::tensor({10, 15, 20}, torch::kInt32);
    torch::Tensor bid_sizes = torch::tensor({100, 200, 300}, torch::kInt32);
    torch::Tensor ask_prices = torch::tensor({25, 30, 35}, torch::kInt32);
    torch::Tensor ask_sizes = torch::tensor({150, 250, 350}, torch::kInt32);
    torch::Tensor customer_ids = torch::zeros({num_markets}, torch::kInt32);
    ...
    for (int32_t i = 0; i < num_markets; ++i) {
        cout << "Market " << i << ": "
             << "Best Bid = " << tensor_item_options_i32(best_bid_px[i]) 
             << " @ " << tensor_item_options_i32(best_bid_sz[i])
             << ", Best Ask = " << tensor_item_options_i32(best_ask_px[i])
             << " @ " << tensor_item_options_i32(best_ask_sz[i]) << endl;
    }
    
    // Verify expected values
    assert(tensor_item_options_i32(best_bid_px[0]) == 10);
    assert(tensor_item_options_i32(best_bid_sz[0]) == 100);
    assert(tensor_item_options_i32(best_ask_px[0]) == 25);
    assert(tensor_item_options_i32(best_ask_sz[0]) == 150);
    
    assert(tensor_item_options_i32(best_bid_px[1]) == 15);
    assert(tensor_item_options_i32(best_ask_px[1]) == 30);
    
    assert(tensor_item_options_i32(best_bid_px[2]) == 20);
    assert(tensor_item_options_i32(best_ask_px[2]) == 35);
...
void TestRingBufferWraparound() {
    ...
    const int32_t num_markets = 1;
    const int32_t max_orders = 10;  // Small buffer to test wraparound
    ...
    for (int32_t i = 0; i < max_orders + 5; ++i) {
        torch::Tensor bid_prices = torch::full({1}, 10 + i, torch::kInt32);
        torch::Tensor bid_sizes = torch::full({1}, 100, torch::kInt32);
        torch::Tensor ask_prices = torch::zeros({1}, torch::kInt32);
        torch::Tensor ask_sizes = torch::zeros({1}, torch::kInt32);
        torch::Tensor customer_ids = torch::full({1}, i % 5, torch::kInt32);
    ...
    torch::Tensor bid_prices = torch::zeros({1}, torch::kInt32);
    torch::Tensor bid_sizes = torch::zeros({1}, torch::kInt32);
    torch::Tensor ask_prices = torch::full({1}, 20, torch::kInt32);
    torch::Tensor ask_sizes = torch::full({1}, 500, torch::kInt32);
    torch::Tensor customer_ids = torch::full({1}, 10, torch::kInt32);
    ...
    auto counts = fills.fill_counts.cpu();
    int32_t fill_count = tensor_item_options_i32(counts[0]);
    assert(fill_count > 0);
...
void TestZeroSizeOrders() {
    ...
    torch::Tensor bid_prices = torch::tensor({10, 20, 30}, torch::kInt32);
    torch::Tensor bid_sizes = torch::tensor({0, 100, 0}, torch::kInt32);  // Zero sizes
    torch::Tensor ask_prices = torch::tensor({15, 0, 35}, torch::kInt32);
    torch::Tensor ask_sizes = torch::tensor({50, 0, 200}, torch::kInt32);  // Zero sizes
    torch::Tensor customer_ids = torch::zeros({num_markets}, torch::kInt32);
    ...
    assert(tensor_item_options_i32(best_bid_px[0]) == NULL_INDEX);
    cout << "✓ Market 0: Zero-size bid correctly ignored" << endl;
    
    // Market 1: Has bid
    assert(tensor_item_options_i32(best_bid_px[1]) == 20);
    cout << "✓ Market 1: Non-zero bid correctly added" << endl;
    
    // Market 2: No bid (size was 0)
    assert(tensor_item_options_i32(best_bid_px[2]) == NULL_INDEX);
...
void TestCrossMarketFillCounts() {
    ...
    const int32_t num_markets = 10;
    VecMarket market(num_markets, 128, 1024, 512);
    auto fills = market.NewFillBatch();
    
    // Create a pattern where even markets get fills, odd markets don't
    torch::Tensor bid_prices = torch::zeros({num_markets}, torch::kInt32);
    torch::Tensor bid_sizes = torch::zeros({num_markets}, torch::kInt32);
    torch::Tensor ask_prices = torch::zeros({num_markets}, torch::kInt32);
    torch::Tensor ask_sizes = torch::zeros({num_markets}, torch::kInt32);
    ...
    customer_ids = torch::ones({num_markets}, torch::kInt32);
    
    market.AddTwoSidedQuotes(bid_prices, bid_sizes, ask_prices, ask_sizes, customer_ids, fills);
    
    // Verify fill pattern
    auto counts = fills.fill_counts.cpu();
    int32_t total_fills = 0;
    
    for (int32_t i = 0; i < num_markets; ++i) {
        if (i % 2 == 0) {
            assert(tensor_item_options_i32(counts[i]) == 1);
            total_fills++;
        } else {
            assert(tensor_item_options_i32(counts[i]) == 0);
        }
...
void TestPriceCrossingLogic() {
    ...
    {
        torch::Tensor bid_prices = torch::full({1}, 50, torch::kInt32);
        torch::Tensor bid_sizes = torch::full({1}, 100, torch::kInt32);
        torch::Tensor ask_prices = torch::full({1}, 50, torch::kInt32);
        torch::Tensor ask_sizes = torch::full({1}, 100, torch::kInt32);
        torch::Tensor customer_ids = torch::zeros({1}, torch::kInt32);
    ...
    {
        torch::Tensor bid_prices = torch::full({1}, 60, torch::kInt32);
        torch::Tensor bid_sizes = torch::full({1}, 50, torch::kInt32);
        torch::Tensor ask_prices = torch::full({1}, 55, torch::kInt32);
        torch::Tensor ask_sizes = torch::full({1}, 50, torch::kInt32);
        torch::Tensor customer_ids = torch::ones({1}, torch::kInt32);
    ...
    {
        torch::Tensor bid_prices = torch::full({1}, 45, torch::kInt32);
        torch::Tensor bid_sizes = torch::full({1}, 100, torch::kInt32);
        torch::Tensor ask_prices = torch::full({1}, 50, torch::kInt32);
        torch::Tensor ask_sizes = torch::full({1}, 100, torch::kInt32);
        torch::Tensor customer_ids = torch::full({1}, 2, torch::kInt32);
        ...
        auto counts = fills.fill_counts.cpu();
        assert(tensor_item_options_i32(counts[0]) == 0);
...
void TestExecutionPrice() {
    ...
    cout << "Test 1: Buy order rests, sell order takes" << endl;
    {
        ...
        torch::Tensor bid_prices = torch::full({1}, 55, torch::kInt32);
        torch::Tensor bid_sizes = torch::full({1}, 100, torch::kInt32);
        torch::Tensor ask_prices = torch::zeros({1}, torch::kInt32);
        torch::Tensor ask_sizes = torch::zeros({1}, torch::kInt32);
        torch::Tensor customer_ids = torch::zeros({1}, torch::kInt32);
        ...
        bid_prices = torch::zeros({1}, torch::kInt32);
        bid_sizes = torch::zeros({1}, torch::kInt32);
        ask_prices = torch::full({1}, 50, torch::kInt32);
        ask_sizes = torch::full({1}, 50, torch::kInt32);
        customer_ids = torch::ones({1}, torch::kInt32);
    ...
    cout << "\nTest 2: Sell order rests, buy order takes" << endl;
    {
        ...
        bid_prices = torch::full({1}, 55, torch::kInt32);
        bid_sizes = torch::full({1}, 50, torch::kInt32);
        ask_prices = torch::zeros({1}, torch::kInt32);
        ask_sizes = torch::zeros({1}, torch::kInt32);
        customer_ids = torch::ones({1}, torch::kInt32);
...
void TestLastPriceTracking() {
    ...
    for (int32_t i = 0; i < num_markets; ++i) {
        assert(tensor_item_options_i32(last_prices[i]) == NULL_INDEX);
    }
    ...
    torch::Tensor bid_prices = torch::tensor({50, 60, 70}, torch::kInt32);
    torch::Tensor bid_sizes = torch::tensor({100, 200, 300}, torch::kInt32);
    torch::Tensor ask_prices = torch::tensor({50, 60, 70}, torch::kInt32);
    torch::Tensor ask_sizes = torch::tensor({50, 100, 150}, torch::kInt32);
    torch::Tensor customer_ids = torch::zeros({num_markets}, torch::kInt32);
    ...
    assert(tensor_item_options_i32(last_prices[0]) == 50);
    assert(tensor_item_options_i32(last_prices[1]) == 60);
    assert(tensor_item_options_i32(last_prices[2]) == 70);
    ...
    bid_prices = torch::tensor({0, 65, 0}, torch::kInt32);
    bid_sizes = torch::tensor({0, 50, 0}, torch::kInt32);
    ask_prices = torch::tensor({0, 65, 0}, torch::kInt32);
    ask_sizes = torch::tensor({0, 25, 0}, torch::kInt32);
    customer_ids = torch::ones({num_markets}, torch::kInt32);
    ...
    assert(tensor_item_options_i32(last_prices[0]) == 50);  // Unchanged
    assert(tensor_item_options_i32(last_prices[1]) == 65);  // Updated
    assert(tensor_item_options_i32(last_prices[2]) == 70);  // Unchanged
...
void TestLastPriceMultipleFills() {
    ...
    for (int32_t i = 0; i < 3; ++i) {
        torch::Tensor bid_prices = torch::full({1}, 50 + i * 5, torch::kInt32);  // 50, 55, 60
        torch::Tensor bid_sizes = torch::full({1}, 100, torch::kInt32);
        torch::Tensor ask_prices = torch::zeros({1}, torch::kInt32);
        torch::Tensor ask_sizes = torch::zeros({1}, torch::kInt32);
        torch::Tensor customer_ids = torch::full({1}, i, torch::kInt32);
    ...
    torch::Tensor bid_sizes = torch::zeros({1}, torch::kInt32);
    torch::Tensor ask_prices = torch::full({1}, 50, torch::kInt32);
    torch::Tensor ask_sizes = torch::full({1}, 300, torch::kInt32);
    torch::Tensor customer_ids = torch::full({1}, 3, torch::kInt32);
    ...
    auto fill_counts = fills.fill_counts.cpu();
    int32_t num_fills = tensor_item_options_i32(fill_counts[0]);
    ...
    auto fill_prices = fills.fill_prices.cpu();
    auto prices_acc = fill_prices.accessor<int32_t, 2>();
    int32_t last_fill_price = prices_acc[0][num_fills - 1];
    
    assert(tensor_item_options_i32(last_prices[0]) == last_fill_price);
...
void TestResetAndLastPrice() {
    ...
    torch::Tensor bid_prices = torch::tensor({45, 55}, torch::kInt32);
    torch::Tensor bid_sizes = torch::tensor({100, 200}, torch::kInt32);
    torch::Tensor ask_prices = torch::tensor({45, 55}, torch::kInt32);
    torch::Tensor ask_sizes = torch::tensor({100, 200}, torch::kInt32);
    torch::Tensor customer_ids = torch::zeros({num_markets}, torch::kInt32);
    ...
    auto last_prices = bbo.last_prices.cpu();
    assert(tensor_item_options_i32(last_prices[0]) == 45);
    assert(tensor_item_options_i32(last_prices[1]) == 55);
    ...
    bid_prices = torch::tensor({30, 40}, torch::kInt32);
    bid_sizes = torch::tensor({50, 100}, torch::kInt32);
    ask_prices = torch::tensor({35, 45}, torch::kInt32);
    ask_sizes = torch::tensor({50, 100}, torch::kInt32);
    customer_ids = torch::zeros({num_markets}, torch::kInt32);
    ...
    bbo.Reset();
    last_prices = bbo.last_prices.cpu();
    assert(tensor_item_options_i32(last_prices[0]) == NULL_INDEX);
    assert(tensor_item_options_i32(last_prices[1]) == NULL_INDEX);
    cout << "✓ BBOBatch reset clears last prices to NULL_INDEX" << endl;
    
    // Execute new trades
    cout << "\nExecuting new trades after reset:" << endl;
    bid_prices = torch::tensor({40, 50}, torch::kInt32);
    bid_sizes = torch::tensor({75, 125}, torch::kInt32);
    ask_prices = torch::tensor({35, 45}, torch::kInt32);
    ask_sizes = torch::tensor({75, 125}, torch::kInt32);
    customer_ids = torch::ones({num_markets}, torch::kInt32);
    ...
    last_prices = bbo.last_prices.cpu();
    assert(tensor_item_options_i32(last_prices[0]) == 40);  // Bid was resting
    assert(tensor_item_options_i32(last_prices[1]) == 50);  // Bid was resting
...
void TestLastPricePersistence() {
    ...
    torch::Tensor bid_prices = torch::tensor({100, 200}, torch::kInt32);
    torch::Tensor bid_sizes = torch::tensor({50, 100}, torch::kInt32);
    torch::Tensor ask_prices = torch::tensor({100, 200}, torch::kInt32);
    torch::Tensor ask_sizes = torch::tensor({50, 100}, torch::kInt32);
    torch::Tensor customer_ids = torch::zeros({num_markets}, torch::kInt32);
    ...
    assert(tensor_item_options_i32(last_prices1[0]) == 100);
    assert(tensor_item_options_i32(last_prices1[1]) == 200);
    ...
    assert(tensor_item_options_i32(last_prices2[0]) == NULL_INDEX);
    assert(tensor_item_options_i32(last_prices2[1]) == NULL_INDEX);
    ...
    bid_prices = torch::tensor({110, 210}, torch::kInt32);
    bid_sizes = torch::tensor({25, 50}, torch::kInt32);
    ask_prices = torch::tensor({110, 210}, torch::kInt32);
    ask_sizes = torch::tensor({25, 50}, torch::kInt32);
    customer_ids = torch::ones({num_markets}, torch::kInt32);
    ...
    assert(tensor_item_options_i32(last_prices1[0]) == 100);  // Unchanged
    assert(tensor_item_options_i32(last_prices1[1]) == 200);  // Unchanged
    assert(tensor_item_options_i32(last_prices2[0]) == 110);  // Updated
    assert(tensor_item_options_i32(last_prices2[1]) == 210);  // Updated
